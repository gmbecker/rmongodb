    ...
    if (dim != R_NilValue && (dims = LENGTH(dim)) > 1) {
        ...
        while (success && depth >= 0) {
            if (ijk[depth] < INTEGER(dim)[depth]) {
                ...
                if (depth < dims - 1) {
                  ...
                  if(is_na[i]) {
                    success = (bson_append_null( _buf, _name ) == BSON_OK);
                    i++;
                  } else
        ...
        if (names == R_NilValue)
            if (len == 1) {
              if(is_na[0]) {
                ...
                for (i = 0; i < len && success; i++) {
                  if(is_na[i]) {
                    success &= (bson_append_null( _buf, numstr(i) ) == BSON_OK);
                  } else
            ...
            if (len == 1) {
                ...
                for (i = 0; i < len && success; i++) {
                  if(is_na[i]) {
                    success &= (bson_append_null( _buf, CHAR(STRING_ELT(names, i)) ) == BSON_OK);
                  } else
