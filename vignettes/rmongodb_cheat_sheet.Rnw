...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\SweaveOpts{concordance=TRUE, eval=TRUE, results=hide}
...
count all elements in collection. Collection name is a namespace with 'database.collection'
<<>>=
...
count element of a special query
<<>>=
mongo.count(mongo, "rmongodb.zips", 
            query='{"state":"AL"}')
@
find one document and returns BSON object. For more JSON query syntax check mongodb tutorials: \href{http://docs.mongodb.org/manual/tutorial/query-documents/}{http://docs.mongodb.org/manual/tutorial/query-documents/}
<<>>=
...
@
directly create R object
<<>>=
mongo.find.all(mongo, "rmongodb.zips", 
              query='{"state":"AL"}')
@
skip first 5 documents, limit query to 10 results and only return special keys
<<>>=
mongo.find.all(mongo, "rmongodb.zips", 
              query='{"state":"AL"}',
              skip=5, limit=10,
...
due to new JSON to BSON functionality no longer required. Since version 1.3 you can directly query with JSON!
<<>>=
...
@
old way still available. chech help files
<<>>=
?mongo.bson
@
...
\href{https://github.com/mongosoup/rmongodb}{https://github.com/mongosoup/rmongodb}
